...
void run_round() {
  ...
  if (battery[0]<=0.0 && winner!=0) { // rat 1 wins
    ...
    winner=1;
    wb_supervisor_set_label(FIRST_LABEL, message, 0.0, 0.0, 0.1, WHITE, 0.0);
    wb_supervisor_set_label(SECOND_LABEL, " ", 0.0, 0.0, 0.1, BLACK, 0.0);
    final_counter++;
    ...
    for (i=0; i<2; i++) {
      ...
      wb_supervisor_set_label(FIRST_LABEL + i, message, 0.0, 0.1*i, 0.1, i?GREEN:RED, 0.0);
    }
    ...
    wb_supervisor_set_label(SECOND_LABEL, message, 0.0, 0.1, 0.1, WHITE, 0.0);
  }
  ...
  if (mode==CONTEST) {
    ...
    if (counter==1) // start the movie
      wb_supervisor_start_movie("movie.avi", 640, 480, 0, 30, 1, false);
