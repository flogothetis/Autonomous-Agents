...
void build_maze(Maze *m) {
  ...
  for (i=0; i<m->width; i++) {
    ...
    place_wall(RATIO * i, RATIO * (-0.5 + m->height), M_PI_2);
  }
  for (i=0; i<m->height; i++) {
    place_wall(-RATIO * 0.5, RATIO * i, 0.0);
    place_wall( RATIO * (-0.5 + m->height), RATIO * i, 0.0);
  }
  ...
  // place grey Lego intervals between the external walls
  for (i=0; i<m->width+1; i++) {
    place_interval(RATIO * (-0.5 + i), -0.5 * RATIO);
    place_interval(RATIO * (-0.5 + i), RATIO * (-0.5 + m->height));
  }
  for (i=1; i<m->height; i++) {
    place_interval(-RATIO * 0.5, RATIO * (-0.5 + i));
    place_interval( RATIO * (-0.5 + m->height), RATIO * (-0.5 + i));
  }

  // place grey Lego intervals between the internal walls
