...
//                April 14, 2014     Adapted the supervisor to the rules for the 2014 edition.
//--------------------------------------------------------------------------------------------
...
static void display() {
  ...
  wb_supervisor_set_label(2, text, 0.51 - 0.015 * strlen(text), 0.1, FONT_SIZE, 0x404040, 0.0); // black
...
static void initialize() {
  ...
  if (match_type != DEMO) {
    ...
    wb_supervisor_start_movie("movie.avi", 480, 360, 0, 75, 1, false);
  }
...
static void move_robot_3d(int robot_index, double tx, double ty, double tz, double alpha) {
  if (robots[robot_index]) {
    ...
    if (robots[robot_index]->model == NAO) {
      ...
      // in DARWIN case we need to add a rotation before
